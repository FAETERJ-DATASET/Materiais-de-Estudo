/*
	...
	Listas encadeadas (Listas dinâmicas) - sem repetição de elementos
*/
...
int remover (TLista *L, int numero)
{
	...
	//se chegou aqui, 'numero' não existe na lista
	return 0;
}
...
{
	//declaração de variáveis
	TLista posAnt, posNovo;
	
	//verificando se os valores são distintos
	if (anterior != novo)
	{
		//buscando a primeira ocorrência de 'anterior'
		posAnt = buscar (L, anterior);
		
		//se o 'anterior' foi encontrado...
		if (posAnt)
		{
			//buscando o elemento 'novo' na lista
			posNovo = buscar (L, novo);
			
			//verificando se 'novo' não existe na lista (para não gerar repetição)
			if (!posNovo)
			{
	...
	return 0; //se chegou aqui, a alteração não foi realizada
}
